...
# Umrechnungsfaktor von cm in Zoll (1 Zoll = 2.54 cm)
cm = 1 / 2.54

# ------------------------
# Global matplotlib style (ersetzt Seaborn)
# ------------------------
plt.rcParams.update({
...
csv_filename = "POT_Dejori_implicit_Chamois_out.csv"
...
# Richtige Spalten zuweisen
displacement = data['displacement_u_y']
reaction_force = data['load_rf_y']

# --- Daten als .txt speichern (Exakt wie bei Abaqus) ---
txt_fn = "RF.txt"
txt_path = os.path.join(script_dir, txt_fn)

# Kombiniere unskalierte Daten, Skalierung passiert erst im Plot!
export_data = np.column_stack((displacement, reaction_force))

# Speichern mit Leerzeichen als Delimiter, analog zu Abaqus
...
# Versuchsdaten zeichnen (grau, strichliert, *1000 für Umrechnung in N)
...
# Simulationsdaten zeichnen (Orange)
